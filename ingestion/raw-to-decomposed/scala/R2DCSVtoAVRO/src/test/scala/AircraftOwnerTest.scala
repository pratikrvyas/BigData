 ...
 * Filename    : AircraftOwnerTest.scala
...
class AircraftOwnerTest extends FunSuite with BeforeAndAfter with SharedSparkContext {
  ...
  before {
    ...
    in_df = sqlContext.read.format("com.databricks.spark.csv").option("delimiter",",").option("header","true").option("inferSchema","true")
      .load("./src/test/resources/AircraftOwnerMaster.csv")
  }
