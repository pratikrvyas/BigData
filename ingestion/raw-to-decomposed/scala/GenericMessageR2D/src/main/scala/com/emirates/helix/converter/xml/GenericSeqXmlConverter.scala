 ...
 *   - It converts the message xml string into a dataframe.
 ...
 *     --class "com.emirates.helix.converter.xml.GenericSeqXmlConverter"
 *     --master yarn \
 *     --jars /home/sarojk/conf/xml_avro/spark-xml_2.10-0.3.5.jar \
 *     /home/sarojk/conf/xml_avro/helix-message-converter.jar ${INPUT_HDFS_PATH} \
 *     ${XML_ROOT_ELEMENT} \
 *     ${AVRO_SCHEMA_FILE} \
 *     ${HDFSURI} \
 *     ${OUTPUT_HDFS_PATH} \
...
object GenericSeqXmlConverter extends App {
  if (args.size < 9) {
    throw new IllegalArgumentException("""All nine parameters -
      [inputPath] [rootElement] [schemaFile] [hdfsUri] [outputPath] [writeMode] [outputFormat] [compression] [noOfPartitions] are mandatory.
  ...
  logger.info("Convert Avro schema to Spark Sql schema started")
  ...
  logger.info("Avro schema to Spark Sql schema completed")

  logger.info("Message conversion started")

  logger.info("STEP:1 -- Converting sequence file to rdd of string")

  val rdd_xml: RDD[String] = readFile(inputPath, sc).cache()

  logger.info("STEP:1 -- Completed")

  logger.info("STEP:2 -- Replacing the namespace and \\n character if passed as argument")

  val rdd_xml_str: RDD[String] = if (nameSpace isDefined) {
    rdd_xml.map(p => p.toString.replace("ns0:", "").replace(":ns0", "").replace("\\n", ""))
  } else {
    rdd_xml.map(p => p.toString.replace("\\n", ""))
  }
  ...
  logger.info("STEP: 3 -- Converting XMLs to " + outputFormat + " with " + " compression " + compression)
  ...
  df_xml match {
    case Failure(df) =>
      ...
      System.exit(1)
    case Success(df) =>
      logger.info("STEP:3 -- XML to dataframe conversion completed")
      logger.info("STEP:4 -- Generate uuid,timestamp and store the dataframe into output hdfs path")
