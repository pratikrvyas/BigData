...
do 
    ...
    do
		...
		DIR=$( grep OMEGA_OUTPUT_PATH ${tab}R2D.properties | cut -d"=" -f2 | cut -d"$" -f1 )
		echo $DIR$START/_SUCCESS
		
		hdfs dfs -test -e $DIR$START/_SUCCESS
		
		if [[ $? -eq 0 || OVERWRITE -eq 1 ]]; then continue; fi
		...
		sed -i "s@START=.*@$NEW_START@g" ${tab}R2D.properties
		sed -i "s@END=.*@$NEW_END@g" ${tab}R2D.properties
		
		#cat ${tab}R2D.properties
		
		oozie_return=$(oozie job -config ${tab}R2D.properties -run)
