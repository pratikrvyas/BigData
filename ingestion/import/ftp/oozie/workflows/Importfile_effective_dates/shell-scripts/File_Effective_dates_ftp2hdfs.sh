...
#       Purpose:          FILE_EFFECTIVE_DATES_Script identifies the incremental files and copies them to HDFS
#
# -----------------------------------------------

HDFS_TARGET_FILE_EFFECTIVE_DATES=$1
CONTROL_FILE=$2
LOCAL_DIR=$3
WRK_DIR=$4
FTP_DETAILS=$5
...
then
       ...
       New_files_not_available="FILE_EFFECTIVE_DATES | FTP connection failed"
...
then
       ...
       New_files_not_available="FILE_EFFECTIVE_DATES | Files are not available. Please check!"
            echo Mail_MSG=$New_files_not_available
                echo "ERRORVAL=FALSE"
...
while read -r line
do
  if [ $line = file_effective_dates.csv ] ;
        then
            echo "file naming convention is correct"

                        else
                        FILE_NAME="file effective dates  | File name convention is incorrect.Please check! "
                                          echo "BODY1=Actual Filename:$line ; Expected Filename:file_effective_dates.csv "
...
then
      FTP_mismatch="FILE_EFFECTIVE_DATES | FTP of few files is not successfull. Please check!"
          echo "ERRORVAL=FALSE"
...
hadoop fs -test -d ${HDFS_TARGET_FILE_EFFECTIVE_DATES}

if [ $? == 0 ]; then
    echo "exists"
        hdfs dfs -rm -r ${HDFS_TARGET_FILE_EFFECTIVE_DATES}
...
if [ $? == 0 ]; then
    ...
    hdfs dfs -mkdir -p ${HDFS_TARGET_FILE_EFFECTIVE_DATES}
fi
...
do
        if [[ $line = file_effective_dates* ]];
        then
             gzip $line
             hdfs dfs -copyFromLocal ${LOCAL_DIR}/$line.gz ${HDFS_TARGET_FILE_EFFECTIVE_DATES}
...
hdfs dfs -touchz ${HDFS_TARGET_FILE_EFFECTIVE_DATES}/_SUCCESS
