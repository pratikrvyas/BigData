...
#       Purpose:          manual airport city_masters_ftp_Script identifies the incremental files and copies them to HDFS
#
# -----------------------------------------------

HDFS_TARGET_DIR_MNL_CITY_MASTER=$1
CONTROL_FILE=$2
LOCAL_DIR=$3
WRK_DIR=$4
FTP_DETAILS=$5
...
then
       ...
       New_files_not_available="MANUAL AIRPORT CITY MASTER| FTP connection failed"
...
then
       ...
       New_files_not_available="MANUAL AIRPORT CITY MASTER | Files are not available. Please check!"
            echo Mail_MSG=$New_files_not_available
                echo "ERRORVAL=FALSE"
...
while read -r line
do
  if [ $line = manual_airport_city_master.csv ] ;
        then
            echo "file naming convention is correct"

                        else
                        FILE_NAME="MANUAL CITY AIRPORT MASTER  | File name convention is incorrect.Please check! "
                                          echo "BODY1=Actual Filename:$line ; Expected Filename:manual_airport_city_master.csv "
...
if grep -q "Not logged in" $WRK_DIR/ftp_log;
   then
   FTP_ISSUE="MANUAL AIRPORT CITY MASTER | FTP connection failed"
...
then
      FTP_mismatch="MANUAL AIRPORT CITY MASTER | FTP of few files is not successfull. Please check!"
          echo "ERRORVAL=FALSE"
...
hadoop fs -test -d ${HDFS_TARGET_DIR_MNL_CITY_MASTER}

if [ $? == 0 ]; then
    echo "exists"
        hdfs dfs -rm -r ${HDFS_TARGET_DIR_MNL_CITY_MASTER}
...
if [ $? == 0 ]; then
    ...
    hdfs dfs -mkdir -p ${HDFS_TARGET_DIR_MNL_CITY_MASTER}
fi
 ...
 ls -ltr * | awk '{ print $9 }' | sed '/.zip/d' | while read line;
         do
           ...
           if [[ $line = manual_airport_city_master* ]];
        then
           gzip $line
             hdfs dfs -copyFromLocal ${LOCAL_DIR}/$line.gz ${HDFS_TARGET_DIR_MNL_CITY_MASTER}
                                                 else
                         echo "do nothing"
                         fi
...
echo "$execution_date;SUCCESS" | hdfs dfs -appendToFile - ${CONTROL_FILE}
 ...
 hdfs dfs -touchz ${HDFS_TARGET_DIR_MNL_CITY_MASTER}/_SUCCESS
