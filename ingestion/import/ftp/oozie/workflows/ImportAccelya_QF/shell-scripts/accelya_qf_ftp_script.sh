...
then
       ...
       FTOP_CONNECTION_FAILED_BODY="Please check FTP connection details and ensure server is available."
            echo BODY1=$FTOP_CONNECTION_FAILED_BODY
                echo "ERRORVAL=FALSE"
...
do
  ...
  then
    ...
    pattern=$var1';'$var2
    echo "$pattern"
    if grep -q $pattern $WRK_DIR/acl_qf_control.txt;
     then
    ...
    echo "$line is a 176 file, not an EKJ"
  fi
