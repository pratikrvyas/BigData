...
do
  var0=${line:0:19}
  if [ $var0 == "2017011911403_daily" ]
  then
    echo "$line is a daily 03 file" >> $WRK_DIR/debug.txt
    echo "$line" >> $WRK_DIR/daily_03_file_list.txt
    var1=${line:20:10}
    echo "$var1" >> $WRK_DIR/03_date_list.txt
  elif [ $var0 == "2017011911402_daily" ]
  then
    echo "$line is a daily 02 file" >> $WRK_DIR/debug.txt
    echo "$line" >> $WRK_DIR/daily_02_file_list.txt
    var1=${line:20:10}
    echo "$var1" >> $WRK_DIR/02_date_list.txt
  else
    echo "$line is not a daily file" >> $WRK_DIR/debug.txt
  fi
  job_rc=$?
  if [[ $job_rc -ne 0 ]];
    then
...
do
  ...
  then
    ...
    grep "$line" daily_03_file_list.txt >> $WRK_DIR/03_file_to_download.txt
    grep "$line" daily_03_file_list.txt >> $WRK_DIR/files_to_download.txt
    break
  else
    echo "$line filename indicates it does not contain data later than $LAST_PROCESSED_DAY. File will not be ingested." >> $WRK_DIR/debug.txt
  fi
  job_rc=$?
  if [[ $job_rc -ne 0 ]];
    then
...
do
  ...
  then
    ...
    grep "$line" daily_02_file_list.txt >> $WRK_DIR/02_file_to_download.txt
    grep "$line" daily_02_file_list.txt >> $WRK_DIR/files_to_download.txt
    break
  else
    echo "$line filename indicates it does not contain data later than $LAST_PROCESSED_DAY. File will not be ingested." >> $WRK_DIR/debug.txt
  fi
