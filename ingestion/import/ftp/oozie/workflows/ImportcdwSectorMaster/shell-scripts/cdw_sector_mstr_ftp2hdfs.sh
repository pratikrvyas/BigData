...
#       Purpose:          cdw_sector_master_ftp_Script identifies the HISTORY files and copies them to HDFS
#
# -----------------------------------------------

HDFS_TGT_DIR_CDW_SECTOR_MASTER=$1
CONTROL_FILE=$2
LOCAL_DIR=$3
WRK_DIR=$4
FTP_DETAILS=$5
...
then
       ...
       New_files_not_available="CDW SECTOR MASTER | FTP connection failed"
...
then
       ...
       New_files_not_available="CDW SECTOR MASTER | Files are not available. Please check!"
            echo Mail_MSG=$New_files_not_available
                echo "ERRORVAL=FALSE"
...
if grep -q "Not logged in" $WRK_DIR/ftp_log;
   then
   FTP_ISSUE="CDW SECTOR MASTER | FTP connection failed"
...
then
      FTP_mismatch="CDW SECTOR MASTER | FTP of few files is not successfull. Please check!"
          echo "ERRORVAL=FALSE"
...
hadoop fs -test -d ${HDFS_TGT_DIR_CDW_SECTOR_MASTER}

if [ $? == 0 ]; then
    echo "exists"
        hdfs dfs -rm -r ${HDFS_TGT_DIR_CDW_SECTOR_MASTER}
...
if [ $? == 0 ]; then
    ...
    hdfs dfs -mkdir -p ${HDFS_TGT_DIR_CDW_SECTOR_MASTER}
fi
...
do
        if [[ $line = cdw_sector_master* ]];
        then
             gzip $line
             hdfs dfs -copyFromLocal ${LOCAL_DIR}/$line.gz ${HDFS_TGT_DIR_CDW_SECTOR_MASTER}
...
echo "$execution_date;SUCCESS" | hdfs dfs -appendToFile - ${CONTROL_FILE}
 ...
 hdfs dfs -touchz ${HDFS_TGT_DIR_CDW_SECTOR_MASTER}/_SUCCESS
