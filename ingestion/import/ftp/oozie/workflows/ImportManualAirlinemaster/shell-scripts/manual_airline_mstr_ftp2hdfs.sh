...
#       Purpose:          manual_airlinemaster_ftp_Script identifies the incremental files and copies them to HDFS
#
# -----------------------------------------------

HDFS_TARGET_DIR_MNL_AIR_MASTER=$1
CONTROL_FILE=$2
LOCAL_DIR=$3
WRK_DIR=$4
FTP_DETAILS=$5
...
then
       ...
       New_files_not_available="MANUAL AIRLINE MASTER | FTP connection failed"
...
then
       ...
       New_files_not_available="MANUAL AIRLINE MASTER | Files are not available. Please check!"
            echo Mail_MSG=$New_files_not_available
                echo "ERRORVAL=FALSE"
...
while read -r line
do
  if [ $line = manual_airline_master.csv ] ;
        then
            echo "file naming convention is correct"

                        else
                        FILE_NAME="MANUAL AIRLINE MASTER  | File name convention is incorrect.Please check! "
                                          echo "BODY1=Actual Filename:$line ; Expected Filename:manual_airline_master.csv "
...
then
      FTP_mismatch="MANUAL AIRLINE MASTER | FTP of few files is not successfull. Please check!"
          echo "ERRORVAL=FALSE"
...
hadoop fs -test -d ${HDFS_TARGET_DIR_MNL_AIR_MASTER}

if [ $? == 0 ]; then
    echo "exists"
        hdfs dfs -rm -r ${HDFS_TARGET_DIR_MNL_AIR_MASTER}
...
if [ $? == 0 ]; then
    ...
    hdfs dfs -mkdir -p ${HDFS_TARGET_DIR_MNL_AIR_MASTER}
fi
...
do
        if [[ $line = manual_airline_master* ]];
        then
             gzip $line
             hdfs dfs -copyFromLocal ${LOCAL_DIR}/$line.gz ${HDFS_TARGET_DIR_MNL_AIR_MASTER}
...
echo "$execution_date;SUCCESS" | hdfs dfs -appendToFile - ${CONTROL_FILE}
 ...
 hdfs dfs -touchz ${HDFS_TARGET_DIR_MNL_AIR_MASTER}/_SUCCESS
