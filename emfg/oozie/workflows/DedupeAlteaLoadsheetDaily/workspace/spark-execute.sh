...
echo spark2-submit --master yarn --deploy-mode cluster --executor-memory ${EXECUTOR_MEMORY} --driver-memory ${DRIVER_MEMORY} --queue ${QUEUE} --conf spark.sql.shuffle.partitions=10 --conf spark.dynamicAllocation.minExecutors=${MIN_EXECUTORS} --conf spark.dynamicAllocation.maxExecutors=${MAX_EXECUTORS} --class ${CLASS_NAME} ${SPARK_JAR} -i ${PAYLOAD_HISTORY_SCHEMA_SYNC_PATH} -j ${PAYLOAD_LOADSHEET_INCREMENTAL_PATH} -x ${PAYLOAD_LOADSHEET_DEDUPE_HISTORY_LOCATION} -y ${PAYLOAD_LOADSHEET_DEDUPE_CURRENT_LOCATION} -p ${COALESCE_VALUE} -m ${LOOKBACK_MONTHS} -n ${DATA_LOAD_MODE} -c ${COMPRESSION}

spark2-submit --master yarn --deploy-mode cluster --executor-memory ${EXECUTOR_MEMORY} --driver-memory ${DRIVER_MEMORY} --queue ${QUEUE} --conf spark.sql.shuffle.partitions=10 --conf spark.dynamicAllocation.minExecutors=${MIN_EXECUTORS} --conf spark.dynamicAllocation.maxExecutors=${MAX_EXECUTORS} --class ${CLASS_NAME} ${SPARK_JAR} -i ${PAYLOAD_HISTORY_SCHEMA_SYNC_PATH} -j ${PAYLOAD_LOADSHEET_INCREMENTAL_PATH} -x ${PAYLOAD_LOADSHEET_DEDUPE_HISTORY_LOCATION} -y ${PAYLOAD_LOADSHEET_DEDUPE_CURRENT_LOCATION} -p ${COALESCE_VALUE} -m ${LOOKBACK_MONTHS} -n ${DATA_LOAD_MODE} -c ${COMPRESSION}
