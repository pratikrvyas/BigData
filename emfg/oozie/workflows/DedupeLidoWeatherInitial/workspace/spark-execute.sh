...
PARTITIONS=${12}
NO_OF_DAYS=${13}
HISTORY_OUTPUT_PATH=${14}

echo spark2-submit --master yarn --deploy-mode cluster --executor-cores 4 --executor-memory ${EXECUTOR_MEMORY} --driver-memory ${DRIVER_MEMORY} --queue ${QUEUE} --conf spark.dynamicAllocation.minExecutors=${MIN_EXECUTORS} --conf spark.dynamicAllocation.maxExecutors=${MAX_EXECUTORS} --class ${CLASS_NAME} ${SPARK_JAR} -i ${INCR_INPUT_PATH} -h ${HIST_INPUT_PATH} -c ${COMPRESSION} -l ${LATEST_OUTPUT_PATH} -p ${PARTITIONS} -d ${NO_OF_DAYS} -o ${HISTORY_OUTPUT_PATH} --init
