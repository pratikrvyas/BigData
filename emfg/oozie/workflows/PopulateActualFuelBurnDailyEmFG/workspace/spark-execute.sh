...
spark2-submit --master yarn --deploy-mode cluster --executor-memory ${EXECUTOR_MEMORY} --driver-memory ${DRIVER_MEMORY} --executor-cores ${EXECUTOR_CORES} --queue ${QUEUE} --conf spark.sql.shuffle.partitions=500 --conf spark.sql.autoBroadcastJoinThreshold=1073741824 --conf spark.dynamicAllocation.enabled=true --conf spark.dynamicAllocation.minExecutors=${MIN_EXECUTORS} --conf spark.dynamicAllocation.maxExecutors=${MAX_EXECUTORS} --class ${CLASS_NAME} ${SPARK_JAR} -i ${INPUT_DEDUPE_CURRENT_PATH} -j ${INPUT_FLIGHT_MASTER_CURRENT_PATH} -x ${OUTPUT_CURRENT_PATH} -y ${OUTPUT_REJECTED_PATH} -c ${COMPRESSION} -p ${PARTITIONS}