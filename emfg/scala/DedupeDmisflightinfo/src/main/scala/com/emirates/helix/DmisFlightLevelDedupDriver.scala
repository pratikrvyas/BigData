 ...
 * Filename    : DmisDedupDriver.scala
 * Description : Scala application main class file for doing the DMIS data dedup in
 *               model layer
 * ----------------------------------------------------------------------------
 */
...
/**
  * DmisDedupDriver class with main method for starting point for the app
  */
object DmisFlightLevelDedupDriver {
  private val compress_rex = """^snappy$|^deflate$""".r
  private val format_rex = """^avro$|^parquet$""".r

  private lazy val logger:Logger = Logger.getLogger(DmisFlightLevelDedupDriver.getClass)
  ...
  def main(args:Array[String]) : Unit = {
    ...
    val parser = new scopt.OptionParser[Model.DMISDedupArgs]("spark-submit <spark-options> --class com.emirates.helix.DmisFlightMasterLoadDriver" +
      " dmis-flighmaster-load-<jar version>.jar") {
      head("DMISFlightMasterLoad")
    ...
    parser.parse(args, Model.DMISDedupArgs()) map { config =>
      val processor = DmisFlightLevelDedupProcessor(config)
      ...
      (config.init, config.h_out_path) match {
        ...
        case (true,Some(_)) => {
          val (cur, hist) = processor.processData(processor.readValueFromHDFS(config.incr_path,"avro",hiveContext),
            processor.readValueFromHDFS(config.hist_path,"avro",hiveContext))
        ...
        case (false,_) => {
          val cur = processor.processData(processor.readValueFromHDFS(config.incr_path,"avro",hiveContext),
            processor.readValueFromHDFS(config.hist_path,"parquet",hiveContext))._1
          processor.writeToHDFS(cur,config.l_out_path,"parquet")
        }
      }
    } getOrElse {
