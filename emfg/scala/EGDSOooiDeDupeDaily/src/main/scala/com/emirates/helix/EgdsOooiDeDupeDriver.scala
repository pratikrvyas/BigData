...
object EgdsOooiDeDupeDriver {
  ...
  /** Main method
    ...
    *              1. egds_oooi_src_dedupe_current_location
    *              2. egds_oooi_src_incremental_location
    *              3. egds_oooi_tgt_dedupe_current_location
    *              4. coalesce_value
    *              5. lookback_months
    *              6. compression
    *  @return Unit
  ...
  def main(args:Array[String]) : Unit = {
    ...
    val parser = new scopt.OptionParser[Args]("spark-submit <spark-options> --class com.emirates.helix.EgdsOooiDeDupeDriver " +
      " egdsoooidedupedaily-<jar version>.jar") {
      head("EgdsOooiDeDupeDaily")
      opt[String]('i', "egds_oooi_src_dedupe_current_location")
        .required()
        .action((x, config) => config.copy(egds_oooi_src_dedupe_current_location = x))
        .text("Required parameter : EGDS Oooi src dedupe current location ")
      ...
      opt[String]('x', "egds_oooi_tgt_dedupe_current_location")
        .required()
    ...
    parser.parse(args, Args()) map { config =>
      ...
      processor.writeEGDSOooiDeDupe(processor.deDupeEGDSOooi(processor.readEGDSOooi),config.lookback_months,
        config.coalesce_value, config.egds_oooi_tgt_dedupe_current_location)
    } getOrElse {
