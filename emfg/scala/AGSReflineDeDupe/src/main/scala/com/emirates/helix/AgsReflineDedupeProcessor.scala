...
class AgsReflineDedupeProcessor {
  ...
  def readAGSRefline(implicit sqlContext: HiveContext): DataFrame = {
    val ags_hist_sync_df = sqlContext.read.format("com.databricks.spark.avro")
      .load(args.ags_src_refline_history_sync_location)
    ...
    val ags_incr_formatted_df = sqlContext.sql("select Refline," +
      ...
      "cast(trim(FUEL_ON) as int)*1000 as FUEL_ON, trim(FileName) as FileName, trim(FolderName) as FolderName, " +
      "HELIX_UUID,HELIX_TIMESTAMP from ags_incr_df_tab")
  ...
  def deDupeAGSRefline(dataFrame: DataFrame)(implicit sqlContext: HiveContext): DataFrame = {
    ...
    val deduped_df = dataFrame.drop("HELIX_UUID").drop("HELIX_TIMESTAMP").dropDuplicates()
  ...
  def writeAGSDeDupe(out_df: DataFrame, lookback_months : Int, coalesce_value : Int,
                     ags_tgt_refline_dedupe_history_location : String,
                     ags_tgt_refline_dedupe_current_location : String) (implicit sqlContext: HiveContext) : Unit = {
    out_df.registerTempTable("ags_refline_temptab")

    val max_flight_date = sqlContext.sql("select max(FltDate) from ags_refline_temptab").first().mkString

    val incr_sql = "select * from ags_refline_temptab where to_date(FltDate) >= " +
      "add_months(to_date('" + max_flight_date + "')," + -1 * lookback_months + ")"
    val hist_sql = "select * from ags_refline_temptab where to_date(FltDate) < " +
      "add_months(to_date('" + max_flight_date + "')," + -1 * lookback_months + ")"
    ...
    output_df_last_hist.write.format("parquet").save(args.ags_tgt_refline_dedupe_history_location)
    output_df_last_incr.write.format("parquet").save(args.ags_tgt_refline_dedupe_current_location)
  }
