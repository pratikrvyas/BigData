...
object AgsRouteDedupeProcessor {
  ...
  /**
    * AgsRouteProcessor instance creation
...
/**
  ...
  * read history and incremental data of AGS Route and perform dedupe.
  */
class AgsRouteDedupeProcessor extends SparkUtils{
  private var args: Args = _

   /**  Method to read the AGS Route historic sync and incremental data and
    *   union the result set
    *  @return DataFrame
    */
  def readAGSRoute(implicit spark: SparkSession): DataFrame = {

    val ags_route_hist_sync_df = readValueFromHDFS(args.ags_src_route_history_sync_location,"avro",spark)
    ...
    val ags_route_incr_formatted_df = spark.sql("select trim(UTC) as UTC, " +
      ...
      " trim(PK_Dest) as PK_Dest, split(trim(FileName), '.gz')[0] as FileName, trim(FolderName) as FolderName," +
      " trim(Altitude) as Altitude," +
    ...
    val ags_route_hist_incr_df = ags_route_hist_sync_df.union(ags_route_incr_formatted_df)
    return ags_route_hist_incr_df
  }

  /**  Method to DeDupe the AGS Route historical and incremental data
    *
    *  @return DataFrame
    */
  ...
  /**  Method to write processed data to HDFS
    ...
    */
  def writeAGSDeDupe(out_df: DataFrame, lookback_months : Int, coalesce_value : Int,
                     ags_tgt_route_dedupe_history_location : String,
                     ags_tgt_route_dedupe_current_location : String) (implicit spark: SparkSession) : Unit = {

    out_df.createOrReplaceTempView("ags_route_deduped_df_tab")

    val max_flight_date = spark.sql("select max(PK_Date) from ags_route_deduped_df_tab").first().mkString

    val incr_sql = "select * from ags_route_deduped_df_tab where to_date(PK_Date) >= " +
      "add_months(to_date('" + max_flight_date + "')," + -1 * lookback_months + ")"
    val hist_sql = "select * from ags_route_deduped_df_tab where to_date(PK_Date) < " +
      "add_months(to_date('" + max_flight_date + "')," + -1 * lookback_months + ") "
    ...
    writeToHDFS(output_df_last_hist,args.ags_tgt_route_dedupe_history_location,"parquet",
      Some("error"),Some(coalesce_value))

    writeToHDFS(output_df_last_incr,args.ags_tgt_route_dedupe_current_location,"parquet",
      Some("error"),Some(coalesce_value))
