...
class ActualFuelBurnProcessor extends SparkUtils with ActualFuelBurnUtils {
  ...
  /** Method to prepare the complex columns and join
    ...
    * @return DataFrame
    */
  def processActualFuelBurn(dataFrame: DataFrame)(implicit spark: SparkSession): DataFrame = {
  ...
  /** Method to write the  data to HDFS
    ...
    */
  def writeActualFuelBurn(out_df: DataFrame, actualfuelburn_tgt_current_location : String, coalesce_value : Int)
                         (implicit spark: SparkSession): Unit = {
    writeToHDFS(out_df, actualfuelburn_tgt_current_location, "parquet",
