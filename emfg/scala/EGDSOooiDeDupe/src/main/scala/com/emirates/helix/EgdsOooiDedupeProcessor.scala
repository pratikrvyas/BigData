...
class EgdsOooiDedupeProcessor extends SparkUtils{
  ...
  def readEGDSOooi(implicit spark: SparkSession): DataFrame = {
    ...
    val egds_oooi_hist_sync_df = readValueFromHDFS(args.egds_oooi_src_history_sync_location,"parquet",spark)
    egds_oooi_hist_sync_df.createOrReplaceTempView("egds_oooi_hist_df_tab")
    ...
    val egds_oooi_hist_formatted_df = spark.sql("select acreg_no, flight_no, flight_date, dep_stn, arr_stn, zfw, " +
      ...
      "trim(msgDeliveredDateTime_FUEL_IN) end as msgDeliveredDateTime_FUEL_IN, OOOI_FUEL_IN, DRVD_FUEL_IN, FUEL_IN, processing_date from egds_oooi_hist_df_tab")
    ...
    val egds_oooi_hist_incr_df = egds_oooi_hist_formatted_df.union(egds_oooi_incr_formatted_df)
  ...
  def writeEGDSOooiDeDupe(out_df: DataFrame, lookback_months : Int, coalesce_value : Int,
                     egds_oooi_tgt_dedupe_history_location : String,
                     egds_oooi_tgt_dedupe_current_location : String)
                    (implicit spark: SparkSession) : Unit = {

    out_df.createOrReplaceTempView("egds_oooi_deduped_df_tab")

    val max_flight_date = spark.sql("select max(flight_date) from egds_oooi_deduped_df_tab").first().mkString

    val incr_sql = "select * from egds_oooi_deduped_df_tab where to_date(flight_date) >= " +
      "add_months(to_date('" + max_flight_date + "')," + -1 * lookback_months + ")"
    val hist_sql = "select * from egds_oooi_deduped_df_tab where to_date(flight_date) < " +
      "add_months(to_date('" + max_flight_date + "')," + -1 * lookback_months + ") "
    ...
    writeToHDFS(output_df_last_hist,args.egds_oooi_tgt_dedupe_history_location,"parquet",
      Some("overwrite"),Some(coalesce_value))

    writeToHDFS(output_df_last_incr,args.egds_oooi_tgt_dedupe_current_location,"parquet",
      Some("overwrite"),Some(coalesce_value))
