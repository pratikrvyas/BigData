...
object Dedup_Lido_Fsum_Flight_Level_Incr {

  private val compress_rex = """^snappy$|^deflate$""".r
  val conf = new SparkConf().setAppName("Dedup_Lido_Fsum_Flight_Level_Incr")
  ...
  def dedup(hist_input_df : DataFrame , incr_input_df : DataFrame) : DataFrame = {
    ...
    val rank_df = hiveContext.sql("select *,row_number() OVER (PARTITION BY cast(trim(FliDupNo) as int),trim(FliNum),trim(DatOfOriUtc),trim(DepAirIat),trim(DesAirIat),trim(RegOrAirFinNum),trim(DepRun),trim(ArrRun),trim(CosIndEcoCru),trim(FirstAltIat),trim(FirstAltIca),trim(SecondAltIat),trim(SecondAltIca),trim(ThirdAltIat),trim(ThirdAltIca),trim(FourthAltIat),trim(FourthAltIca) ORDER BY trim(tibco_message_time) DESC) as rank from drop_dup_tbl")
