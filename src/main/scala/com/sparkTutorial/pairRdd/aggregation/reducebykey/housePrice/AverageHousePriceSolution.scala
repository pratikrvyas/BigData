...
object AverageHousePriceSolution {
  ...
  def main(args: Array[String]) {
    ...
    println("===== Start Debug ====")
    for (line <- cleanedLines.collect().take(5)) println(line)
    println("housePricePairRdd: ")
    for ((x, y) <- housePricePairRdd.collect().take(5)) println(x + " : " + y)
    for ((x,y) <- housePricePairRdd.collect().take(5)) println(x + " : " + y._1+ " : " + y._2)
